 ...
 */
export function anthropicNameToCanonical(name: string): string {
 ...
 */
export function getModelMarketingName(modelId: string): string | undefined {
  const lower = modelId.toLowerCase()
  const has1m = lower.includes('[1m]')
  const base = lower.replace(/\[(1|2)m\]/gi, '').trim()
  const canonical = anthropicNameToCanonical(base)

  if (canonical.includes('claude-opus-4-6'))   return has1m ? 'Opus 4.6 (1M context)' : 'Opus 4.6'
  ...
  if (canonical.includes('claude-sonnet-4-6')) return has1m ? 'Sonnet 4.6 (1M context)' : 'Sonnet 4.6'
  if (canonical.includes('claude-sonnet-4-5')) return has1m ? 'Sonnet 4.5 (1M context)' : 'Sonnet 4.5'
  if (canonical.includes('claude-sonnet-4'))   return has1m ? 'Sonnet 4 (1M context)' : 'Sonnet 4'
  if (canonical.includes('claude-3-7-sonnet')) return 'Sonnet 3.7'
  if (canonical.includes('claude-3-5-sonnet')) return 'Sonnet 3.5'
  if (canonical.includes('claude-haiku-4-5'))  return 'Haiku 4.5'
  if (canonical.includes('claude-3-5-haiku'))  return 'Haiku 3.5'
  return undefined
}
