...
func (c *BedrockClient) Chat(ctx context.Context, messages []EyrieMessage, opts ChatOptions) (*EyrieResponse, error) {
	...
	defer func() { if err := resp.Body.Close(); err != nil { slog.Warn("bedrock: close response body", "error", err) } }()
...
func (c *BedrockClient) StreamChat(ctx context.Context, messages []EyrieMessage, opts ChatOptions) (*StreamResult, error) {
	...
	if resp.StatusCode != http.StatusOK {
		defer func() { if err := resp.Body.Close(); err != nil { slog.Warn("bedrock: close error response body", "error", err) } }()
		detail, readErr := parseProviderError(resp.Body)
	...
	go func() {
		defer close(ch)
		defer cancel()
		defer func() { if err := resp.Body.Close(); err != nil { slog.Warn("bedrock: close stream response body", "error", err) } }()
