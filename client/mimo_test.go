...
func TestMiMoClient_ChatFallsBackToAnthropicOnParamIncorrect(t *testing.T) {
	...
	c.pair.OpenAI.httpClient = &http.Client{Transport: openAITransport}
	c.pair.Anthropic.httpClient = &http.Client{Transport: anthropicTransport}
...
func TestGetOrCreateProvider_XiaomiTokenPlanUsesMimoBase(t *testing.T) {
	...
	if mimo.pair.OpenAI.baseURL != xiaomi.TokenPlanSGPOpenAIBase {
		t.Fatalf("openAI baseURL = %q, want %q", mimo.pair.OpenAI.baseURL, xiaomi.TokenPlanSGPOpenAIBase)
	}
	if mimo.pair.Anthropic == nil || mimo.pair.Anthropic.baseURL != xiaomi.TokenPlanSGPAnthropicBase {
		t.Fatalf("anthropic baseURL = %#v, want %q", mimo.pair.Anthropic, xiaomi.TokenPlanSGPAnthropicBase)
	}
}
